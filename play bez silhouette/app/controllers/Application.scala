...
class Application @Inject() (parcelsDAO: ParcelsDAO, productsDAO: ProductsDAO, orderDAO: OrderDAO, categoriesDAO: CategoriesDAO, cartDAO: CartDAO, registeredUserDAO: RegisteredUserDAO, usersDAO: UsersDAO, testDAO: TestDAO) extends Controller {
  ...
  */
  ...
  def removeuser(id: Int) = Action { implicit request =>
    usersDAO.remove(id)
    Ok
  }
  ...
  def gu = Action.async { implicit request =>
    usersDAO.all map {
      gu => Ok(Json.toJson(gu))
    }
  }
